...
export class MoviesPage implements OnInit {
  ...
  async editMovie(index: number) {
    const movie = this.movies[index];
    const newMovieName = prompt('Enter new movie name:', movie.name);
    if (newMovieName) {
      const newReleaseYear = prompt('Enter new release year:', movie.year);
      if (newReleaseYear) {
        this.movies[index] = { name: newMovieName, year: newReleaseYear };
        try {
          await this.storageService.set('movies', this.movies);
          this.errorMessage = '';
        } catch (error) {
